...
if plan == 'Both':
  adjusted_df = df.loc[((df['Churn_True_Prediction_Score'] >= threshold[0])&(df['Churn_True_Prediction_Score'] <= threshold[1])&(df['State'].isin(states))   )]
else:
  adjusted_df = df.loc[((df['Churn_True_Prediction_Score'] >= threshold[0])&(df['Churn_True_Prediction_Score'] <= threshold[1])&(df['International_Plan'] == 'yes' if plan == "International" else df['International_Plan'] == 'no') &(df['State'].isin(states)))]
...
with graph_col1:
  ...
  cust_service_calls= adjusted_df.groupby(by='Number_Customer_Service_Calls').count().rreset_index()
  fig,ax=plt.subplots()   
